    ...
    def datosManga():
        ...
        def confirmarCompra():
            if not carrito.get_children():
                messagebox.showwarning("Carrito vacion")
                return
        ...
        def dobleClickEliminar(event):
            item = carrito.selection()  # Obtiene la fila seleccionada en ese momento
            if item:                    # Si hay una fila seleccionada
                carrito.delete(item)   # La elimina del Treeview
            else:
